...
def product_detail(request,slug,id):
    ...
    related_products=Product.objects.filter(category=product.category).exclude(id=id)[:4] # show related products
    colors=ProductAttribute.objects.filter(product=product).values('color__id','color__title','color__color_code').distinct()#for product price
...
def delete_cart_item(request):
    ...
    for p_id,item in request.session['cartdata'].items():
        total_amt+=int(item['qty'])*float(item['price'])
    t=render_to_string('ajax/cart-list.html',{'cart_data': request.session['cartdata'],'totalitems':len(request.session[
